...
def draw_front_card(c, x=0, y=0):
    ...
    if (fw, fh) != (1500, 1050):
        raise ValueError(
            f"Postcard_05_Rouen_Front.png is {fw}x{fh}, expected 1500x1050 (PC-13, "
...
def draw_back_card(c, x=0, y=0):
    ...
    # Publisher's imprint -- pure flavour, no referent (PZ-10). Letters used elsewhere: A
    # card01, C card03, D card06, E card07, F the real F1 pointer (card02), G card08. B is
    # free.
    c.setFont("Helvetica", 5)
    c.drawRightString(right, credit_top, "Vinland Editions  ·  Series B, No. 5")
    c.restoreState()
...
def build_single():
    ...
    c.setTitle("Aunt Liv's Postcard 05 - Rouen")
...
def build_letter():
    ...
    c.setTitle("Two-up Postcard 05 print sheet - Rouen")
    c.setAuthor("Escape Backpack")
