...
OUT = ROOT / "output" / "pdf" / "Postcard_H6_Patara_Print.pdf"
OUT_LETTER = ROOT / "output" / "pdf" / "Postcard_H6_Patara_Letter_Print.pdf"
FRONT = ROOT / "NorseBackpack" / "Postcards" / "Postcard_H6_Patara_Front.png"
...
def draw_front_card(c, x=0, y=0):
    ...
    if (fw, fh) != (1500, 1050):
        raise ValueError(
            f"Postcard_H6_Patara_Front.png is {fw}x{fh}, expected 1500x1050 (PC-13, "
...
def draw_back_card(c, x=0, y=0):
    ...
    # Postmark: place only. No date drawn (PC-03/PC-04).
    ...
    c.drawCentredString(postmark_x, postmark_y - 1, "PATARA")
    ...
    # Fun Fact -- typed, real trivia (PC-12). Ties Patara's real 2018 sword find directly to
    # Harald: he served in the Varangian Guard himself, so the sword's owner plausibly walked
    # a path adjacent to his -- not proven to be his own, but genuine Viking-era evidence here.
    ...
    credit_lines = [
        "Sourced via Daily Sabah, 2018 (see stops.js pataraSword).",
    ]
...
def build_single():
    ...
    c.setTitle("Aunt Liv's Postcard H6 - Patara")
...
def build_letter():
    ...
    c.setTitle("Two-up Postcard H6 print sheet - Patara")
    c.setAuthor("Escape Backpack")
