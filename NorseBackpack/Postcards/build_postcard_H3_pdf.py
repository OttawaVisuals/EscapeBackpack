...
OUT = ROOT / "output" / "pdf" / "Postcard_H3_Kyiv_Print.pdf"
OUT_LETTER = ROOT / "output" / "pdf" / "Postcard_H3_Kyiv_Letter_Print.pdf"
FRONT = ROOT / "NorseBackpack" / "Postcards" / "Postcard_H3_Kyiv_Front.png"
...
def draw_front_card(c, x=0, y=0):
    ...
    if (fw, fh) != (1500, 1050):
        raise ValueError(
            f"Postcard_H3_Kyiv_Front.png is {fw}x{fh}, expected 1500x1050 (PC-13, "
...
def draw_back_card(c, x=0, y=0):
    ...
    # Message text approved in chat (PZ-20, 17 Sept 2026): a classic definition riddle whose
    # unstated answer is HONEY -- one third of the MEAD riddle. Answer is never written on the card.
    ...
    c.drawCentredString(postmark_x, postmark_y - 1.5, "KYIV")
    ...
    # Fun Fact -- typed, real trivia (PC-12), deliberately NOT honey/bees: the riddle above is
    # already that content, and a Fun Fact repeating its own answer would break the one-fact
    # rule. Ties Kyiv to Constantinople (HD) through real Rus'-Byzantine history instead.
    ...
    credit_lines = [
        'Image adaptation: "Kyiv St.Michael’s Golden-Domed Monastery" - Rbrechko, CC BY-SA 4.0.',
    ]
...
def build_single():
    ...
    c.setTitle("Aunt Liv's Postcard H3 - Kyiv")
...
def build_letter():
    ...
    c.setTitle("Two-up Postcard H3 print sheet - Kyiv")
    c.setAuthor("Escape Backpack")
